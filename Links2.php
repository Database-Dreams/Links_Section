...
function ViewLinksMain()
{
	...
	GetParentLink($cat);
		...
		$artist = (int) $_REQUEST['art'];
		else
		$artist = 0;
		
		if ($artist != 0){
		$artistExtra = 	' AND art_id = ' . $artist;
		$artist_extra = ' AND l.art_id = ' . $artist;
		}
		...
		$dbresult = $smcFunc['db_query']('', '
			...
			WHERE ID_CAT = {int:this_cat}
				...
				. $artistExtra
				. (isset($_REQUEST['searchresult']) && isset($_REQUEST['searchcat']) && !empty($_REQUEST['searchcat']) ? ' AND CONCAT(title,description) LIKE "%' . $_REQUEST['searchcat'] . '%"' : ''),
			array(
				'this_cat' => $cat,
				'this_approved' => 1
			)
		...
		$dbresult = $smcFunc['db_query']('', '
			SELECT l.ID_LINK,l.title,l.date, l.pagerank, l.description,
			...
			WHERE l.ID_CAT = {int:this_cat}
				...
				. $artist_extra . (isset($_REQUEST['searchresult']) && isset($_REQUEST['searchcat']) && !empty($_REQUEST['searchcat']) ? ' AND CONCAT(l.title,l.description) LIKE "%' . $_REQUEST['searchcat'] . '%"' : '') . '
			ORDER BY l.title LIMIT {int:this_start},{int:this_items_per_page}',
			array(
				'this_cat' => $cat,
				'this_approved' => 1,
...
function AddCat2()
{
    ...
    $smcFunc['db_query']('', "INSERT INTO {db_prefix}links_cat 
			(title, description,roworder,image,ID_PARENT,art_id)
		VALUES ('$title', '$description','$order','$image','$parent','$artist')");
		
	// Get the Category ID
	$cat_id = $smcFunc['db_insert_id']('{db_prefix}links_cat', 'ID_CAT');	

	//Update The Artist If The Parent Cat If One Used
	if ($parent == 4) // Update For Charts Site To 35
	{
		if ($artist != 0)
		{
				// Update the category
		$smcFunc['db_query']('', "UPDATE {db_prefix}artists
		SET artist_links_id = '$cat_id' 
...
function EditCat()
{
	...
	$dbresult = $smcFunc['db_query']('', '
		SELECT ID_CAT, title, description, image, ID_PARENT, art_id 
		FROM {db_prefix}links_cat 
		WHERE ID_CAT = ' . $cat
...
function EditCat2()
{
    ...
    $smcFunc['db_query']('', "UPDATE {db_prefix}links_cat
		SET title = '$title', ID_PARENT = '$parent', description = '$description', image = '$image', art_id = '$artist' WHERE ID_CAT = $catid LIMIT 1");
...
function AddLink2()
{
	global $user_info, $smcFunc, $txt, $sourcedir, $scripturl;
	...
	$AdminLnk = 'https://www.databasedreams.co.uk/charts';
	...
	if ($approved){
		...
		redirectexit('action=links;cat=' . $catid . ';art=' . $artist);	
	}else{
...
function EditLink()
{
	...
	$dbresult = $smcFunc['db_query']('', '
		SELECT ID_LINK, title, ID_CAT, description, url, ID_MEMBER, art_id, official
		FROM {db_prefix}links
...
function EditLink2()
{
	...
	$smcFunc['db_query']('', '
		...
		SET
			...
			art_id = {int:this_artist},
			official = {int:this_official},
			ID_CAT = {int:this_cat}
		WHERE ID_LINK = {int:this_id}
		LIMIT 1',
		array(

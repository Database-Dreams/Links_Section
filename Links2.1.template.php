...
{
	global $scripturl, $context, $txt, $settings, $modSettings, $user_info, $subcats_linktree, $sourcedir;
  ...
  <br />
		...
		foreach($context['links_cats'] as $row)
		{
			...
			if ($m_cats)
				echo '<td class="windowbg2"><a href="' . $scripturl . '?action=links;sa=catup;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txtup'] . '</a>&nbsp;<a href="' . $scripturl . '?action=links;sa=catdown;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txtdown'] . '</a></span><br><a href="' . $scripturl . '?action=links;sa=catperm;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txt_perm'] . '</a>&nbsp;<a href="' . $scripturl . '?action=links;sa=editcat;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txtedit'] . '</a>&nbsp;<a href="' . $scripturl . '?action=links;sa=deletecat;cat=' . $row['ID_CAT'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['smflinks_txtdel'] . '</a></td>';
...
';
  ...
  ';
	...
	else
	{
		...
		$art = (int) $_REQUEST['art'];
		//Get The Header
		$artist_banner = '<div class="windowbg">' .GetArtistBanner($art) .  '</div>';
		$cattitle = $context['art_name'] . ' Links';
		$txt['smflinks_nolinks'] = 'There are no links for ' . $context['art_name'];
		}
	...
	$context['page_index'] = constructPageIndex($scripturl . '?action=links;cat=' . $cat . (isset($_REQUEST['searchresult']) && isset($_REQUEST['searchcat']) && !empty($_REQUEST['searchcat']) ? ';searchresult;searchcat=' . $_REQUEST['searchcat'] : '') , $context['start'], $total, $modSettings['smflinks_setlinksperpage']);
  
  If ($cat == 4 && $art == 0) {
        echo '
         <div><div><form method="post" action="', $scripturl, '?action=links;cat=' . $cat. ';searchresult" class="floatright">
	...
	echo '<div>' . $context['page_index'] . '</div>'.$dv;
		...
		<thead>
		<tr class="title_bar">
						<th scope="col" class="smalltext first_th">' . $txt['smflinks_ctitle'] . '</th>';
						...
						{
							echo '<th class="centertext" align="center">' . $txt['smflinks_chits'] . '</th>';
							$numofspans++;
						}

						if (!empty($modSettings['smflinks_disp_rating']))
						{
							echo '<th  class="centertext" align="center">' . $txt['smflinks_crating'] . '</th>';
							$numofspans++;
						}
						if (!empty($modSettings['smflinks_disp_membername']))
						{
							echo '<th  class="centertext" align="center">' . $txt['smflinks_cusername'] . '</th>';
							$numofspans++;
						}
						if (!empty($modSettings['smflinks_disp_date']))
						{
							echo '<th  class="centertext" align="center">' . $txt['smflinks_cdate'] . '</th>';
							$numofspans++;
						}
			...
			if ($addlink)
			{
				$art_ext = ($art != 0 ? ';art=' . $art : '');
				echo '<a href="' . $scripturl . '?action=links;sa=addlink;cat=' . $cat . $art_ext . '">' . $txt['smflinks_addlink'] . '</a>&nbsp; | ';
...
<table cellspacing="0" align="center" cellpadding="4" class="tborder" width="100%">
  ...
  <tr>
    <td width="28%" class="windowbg2" align="right"><strong>' . $txt['smflinks_artists'] .'</strong>&nbsp;</td>
    <td width="72%" class="windowbg2">' . GetArtistList(0) .'</td>	
  </tr>
  <tr>
    <td width="28%"   class="windowbg2" align="right"><strong>' . $txt['smflinks_description'] . '</strong></td>
...
<table cellspacing="0" align="center" cellpadding="4" class="tborder"  width="100%">
 ...
 <tr>
  ...
  <tr>
    <td width="28%" class="windowbg2" align="right"><strong>' . $txt['smflinks_artists'] .'</strong>&nbsp;</td>
    <td width="72%" class="windowbg2">' . GetArtistList($row['art_id']) .'</td>	
  </tr>
  <tr>
    <td width="28%" class="windowbg2" align="right"><strong>' . $txt['smflinks_description'] . '</strong></td>
    <td width="72%" class="windowbg2"><table>
...
echo '</select>
  ...
  <tr>
    <td width="28%" class="windowbg2"><strong>' . $txt['smflinks_artists'] .'</strong>&nbsp;</td>
    <td width="72%" class="windowbg2">' . GetArtistList($_REQUEST['art']) .'</td>	
  </tr>
  <tr>
  ...
  <tr>
  <td width="28%"   class="windowbg2"><strong>' . $txt['smflinks_official_artists'] .'</strong>&nbsp;</td>
	<td width="72%" class="windowbg2">
		<select name=link_type value="">Link Type</option>
		<option value="0" selected="selected">(None)</option>
		<option value="1">Official Site</option>
		<option value="2">Social Media</option>
		<option value="10">Fan Site</option>
		<option value="11">Fan Forum</option>
		<option value="5">Music Database</option>
		<option value="6">Official Chart Site</option>
		<option value="9">Hobbyist Chart Site</option>
...
{
	global $context, $scripturl, $txt, $modSettings, $settings, $sourcedir;
...
echo '</select>
  ...
  <tr>
    <td width="28%" class="windowbg2"><strong>' . $txt['smflinks_artists'] .'</strong>&nbsp;</td>
    <td width="72%" class="windowbg2">' . GetArtistList($context['links_link']['art_id']) .'</td>	
  </tr>
  <tr>
    <td width="28%"   class="windowbg2"><strong>' . $txt['smflinks_url'] . '</strong></td>
  ...
  <td width="28%"   class="windowbg2"><strong>' . $txt['smflinks_official_artists'] .'</strong>&nbsp;</td>
	<td width="72%" class="windowbg2">
		<select name=link_type value="">Link Type</option>
		<option value="0"' . (($context['links_link']['official'] == 0) ? ' selected="selected"' : '') . '>(None)</option>
		<option value="1"' . (($context['links_link']['official'] == 1) ? ' selected="selected"' : '') . '>Official Site</option>
		<option value="2"' . (($context['links_link']['official'] == 2) ? ' selected="selected"' : '') . '>Social Media</option>
		<option value="10"' . (($context['links_link']['official'] == 10) ? ' selected="selected"' : '') . '>Fan Site</option>
		<option value="11"' . (($context['links_link']['official'] == 11) ? ' selected="selected"' : '') . '>Fan Forum</option>
		<option value="5"' . (($context['links_link']['official'] == 5) ? ' selected="selected"' : '') . '>Music Database</option>
		<option value="6"' . (($context['links_link']['official'] == 6) ? ' selected="selected"' : '') . '>Official Chart Site</option>
		<option value="9"' . (($context['links_link']['official'] == 9) ? ' selected="selected"' : '') . '>Hobbyist Chart Site</option>
		<option value="8"' . (($context['links_link']['official'] == 8) ? ' selected="selected"' : '') . '>Encyclopedia</option>
		<option value="15"' . (($context['links_link']['official'] == 15) ? ' selected="selected"' : '') . '>Other</option></select>
	</td>
  </tr>
  <tr>
    <td width="28%"   valign="top" class="windowbg2"><strong>' . $txt['smflinks_description'] . '</strong></td>
